// Auth Stuff
export const LOGIN_USER_SUCCESS = 'login-user-success'; // Need to be replaced with status
export const LOGIN_USER_ERROR = 'login-user-error'; // Need to be replaced with status
export const LOGIN_USER = 'login-user';
export const CACHE_USER = 'cache-user';

export const UPDATE_USER = 'update-user';
export const FORGOT_PASSWORD = 'forgot-password'; // Currently not used right now
export const LOGOUT_USER = 'logout-user';

// Status Stuff
export const CLEAR = 'clear';
export const SET_ERROR = 'set-error';
export const SET_LOADING = 'set-loading';
export const SET_SUCCESS = 'set-success';

// Petition Stuff
export const GET_TIMELINE = 'get-timeline';
export const GET_ACTIVITY = 'get-activity';
export const GET_PETITIONS = 'get-petitions';
export const GET_SEARCH = 'get-search';
export const GET_SELECTED_PROFILE = 'get-selected-profile';
export const GET_SELECTED_ACTIVITY = 'get-selected-activity';

// Tag stuff
export const GET_TAGS = 'get-tags';
export const NEW_TAG = 'new-tag';

// User Stuff
export const GET_FRIEND_REQUESTS = 'get-friend-requests';
export const GET_FRIENDS = 'get-friends';
export const GET_RECOMMENDED_USERS = 'get-recommended-users';
